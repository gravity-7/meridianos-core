# F006 ΓÇô Azure DevOps Connector

**Feature ID:** F006
**Area:** Integrations
**Wedge:** Multi-Tool Integration Hub (Wedge 3)
**Status:** Ready for Implementation
**Priority:** P0 ΓÇö Bootstrap (unlocks all other features)
...
### Problem
MeridianOS's scheduler/planner/launcher loop works against a local SQLite board (`.ai/state/board.json`). Features must be manually created as tasks on this board. For the self-build strategy, we need MeridianOS to autonomously pull work items from Azure DevOps ΓÇö the source of truth for feature planning ΓÇö and convert them into executable MeridianOS tasks. Without this connector, the mos-dev tenant has nothing to schedule.
...
4. State transitions in MeridianOS (in-review ΓåÆ done) are reflected in ADO
...
- `task.id` ΓåÉ `ADO-<ADO-workitem-id>` (e.g., ADO #42 ΓåÆ `ADO-42`)
- `task.title` ΓåÉ ADO `System.Title`
- `task.spec` ΓåÉ ADO `System.Description` (written to `.ai/features/ADO-<id>/spec.md`)
- `task.acceptance_criteria` ΓåÉ ADO `Microsoft.VSTS.Common.AcceptanceCriteria`
- `task.status` ΓåÉ mapped from ADO state: `Proposed`ΓåÆ`proposed`, `Active`/`Doing`/`In Progress`ΓåÆ`designing`/`spec` (based on state mapping), `Resolved`ΓåÆ`in-review`, `Closed`ΓåÆ`done`
- `task.owner` ΓåÉ from ADO tags (e.g., `builder`, `designer`, `reviewer`, `docs-writer`) or assignee.
...
State mapping between ADO Γåö MeridianOS:
```
ADO: Proposed        ΓåÆ  MeridianOS: proposed
ADO: Active          ΓåÆ  MeridianOS: spec (or designing)
ADO: Resolved        ΓåÆ  MeridianOS: in-review
ADO: Closed          ΓåÆ  MeridianOS: done
ADO: Removed         ΓåÆ  MeridianOS: archived (or deleted)
```

### FR5: Incremental Sync
The connector SHALL be idempotent:
- Running twice does NOT create duplicate tasks
- Task identity is keyed on ADO work item ID (`ADO-{id}`)
...
```js
// azure-devops-source.mjs ΓÇö ADO work item ΓåÆ MeridianOS task bridge
export async function syncFromAdo({ store, config, policy }) ΓåÆ Promise<{created: number, updated: number, skipped: number, errors: string[]}>
export async function pushPrLink({ org, project, pat, id, prUrl, state }) ΓåÆ Promise<void>
export function mapAdoState(state) ΓåÆ string
export function mapAiosStateToAdo(status) ΓåÆ string
```

### TR2: ADO REST API Usage
The connector SHALL use the Azure DevOps REST API (not Git, not Boards SDK):
- List work items: `POST https://dev.azure.com/{org}/{project}/_apis/wit/wiql?api-version=7.1-preview.2`
- Get work item details: `GET https://dev.azure.com/{org}/{project}/_apis/wit/workitems/{id}?$expand=all&api-version=7.1-preview.3`
- Update work item: `PATCH https://dev.azure.com/{org}/{project}/_apis/wit/workitems/{id}?api-version=7.1-preview.3`
- Add comment: (can be posted in history fields or via workitems update API)
...
### TR4: PAT Security
- PAT is read from `process.env.ADO_PAT` (or the configured `pat_env`) ΓÇö NEVER stored in policy.yaml, never committed
- PAT is only used server-side (in the scheduler/daemon process)
- PAT scope: `Work Items (Read, Write)` ΓÇö minimum required permissions
...
ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ                   ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                                scheduler tick (~60s)
                                     Γöé
                                     Γû╝
                               ...
                               GET /_apis/wit/workitems/{id}
                                     Γöé
                                     Γû╝
                                map ADO ΓåÆ MeridianOS tasks
                                write spec files
                                upsert board tasks via store.state.upsertTask()
                                     Γöé
                                     Γû╝
...
### Integration Point
The ADO sync runs as part of the scheduler's planner cycle ΓÇö BEFORE the planner evaluates the board for promotable tasks. This ensures newly imported features are available for agent assignment in the same tick.

---

## Database Changes

**None.** ADO work items become standard MeridianOS tasks stored in the existing SQLite board DB. The spec files are written to `.ai/features/ADO-<id>/spec.md`.
...
- [ ] `syncFromAdo()` returns correct number of Features from the live ADO board
...
### Unit Tests
- `mapAdoState`: verify all state mappings
...
1. Γ£à Running the scheduler in mos-dev pulls Feature work items from `dev.azure.com/qaisarit/meridianOS`
2. Γ£à Each Feature becomes a MeridianOS task on the mos-dev board with correct title, spec, ACs, and state
3. Γ£à Running sync twice does NOT create duplicate tasks
4. Γ£à After an agent opens a PR, the ADO work item is updated with the PR link and state change
5. Γ£à Missing or invalid PAT causes a logged warning, NOT a crash
6. Γ£à Network failures are retried, then gracefully skipped
...
export async function pushPrLink({ org, project, pat, id, prUrl, state })
export function mapAdoState(adoState)
export function toTask(adoItem, config)
```

### Step 3: Wire into scheduler
In `scheduler.mjs`, add a sync call before the planner runs:
```js
if (policy.integrations?.azure_devops?.enabled) {
  try {
    await syncFromAdo({ store, config: cfg, policy });
  } catch (syncErr) {
    logger.error('scheduler', `ADO sync error: ${syncErr.message}`, syncErr);
  }
}
```
...
- `azure-devops-source.mjs` ΓÇö new module
- `scheduler.mjs` ΓÇö add ADO sync before planner cycle
- `runner.mjs` ΓÇö add PR write-back after successful run
- `mos-dev/.ai/policy.yaml` ΓÇö add `integrations.azure_devops` config

### Do NOT
- Hardcode the PAT ΓÇö always read from `process.env[pat_env]`
...
*Feature spec version: 1.1 | Updated: 2026-07-19 | Finalized for implementation*
