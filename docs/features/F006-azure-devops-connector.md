...
- `task.id` ← `ADO-<ADO-workitem-id>` (e.g., ADO #42 → `ADO-42`)
- `task.title` ← ADO `System.Title`
- `task.spec` ← ADO `System.Description` (written to `.ai/features/ADO-<id>/spec.md`)
- `task.acceptance_criteria` ← ADO `Microsoft.VSTS.Common.AcceptanceCriteria`
- `task.status` ← mapped from ADO state: `Proposed`→`proposed`, `Active`/`Doing`/`In Progress`→`designing`/`spec` (based on state mapping), `Resolved`→`in-review`, `Closed`→`done`
- `task.owner` ← from ADO tags (e.g., `builder`, `designer`, `reviewer`, `docs-writer`) or assignee.
...
export async function syncFromAdo({ store, config, policy }) → Promise<{created: number, updated: number, skipped: number, errors: string[]}>
export async function pushPrLink({ org, project, pat, id, prUrl, state }) → Promise<void>
export function mapAdoState(state) → string
export function mapAiosStateToAdo(status) → string
```
