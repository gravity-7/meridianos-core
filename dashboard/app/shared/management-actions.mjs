...
export function typedConfirmation({ title, instruction, confirmLabel, opener, onConfirm }) {
  const dialog = document.createElement('dialog'); dialog.setAttribute('aria-label', title); const field = make('input'); field.id = 'management-confirmation'; field.type = 'text'; field.autocomplete = 'off'; const password = make('input'); password.id = 'management-reauthentication'; password.type = 'password'; password.autocomplete = 'current-password'; password.required = true; const submit = make('button', confirmLabel); submit.type = 'submit'; const form = make('form'); form.method = 'dialog'; const confirmationLabel = make('label', 'Typed confirmation'); confirmationLabel.htmlFor = field.id; const passwordLabel = make('label', 'Reauthentication password'); passwordLabel.htmlFor = password.id; form.append(make('h2', title), make('p', instruction), confirmationLabel, field, passwordLabel, password, submit); form.addEventListener('submit', async (event) => { event.preventDefault(); try { const reauth = await managementRequest('/api/management/reauthenticate', { method: 'POST', body: { password: password.value, localSessionProof: window.AIOS_TOKEN } }); password.value = ''; await onConfirm(field.value, reauth.reauthToken); dialog.close(); } catch (error) { password.value = ''; form.append(notice(error.message, { error: true })); field.focus(); } }); dialog.addEventListener('close', () => { password.value = ''; dialog.remove(); opener?.focus(); }, { once: true }); dialog.append(form); document.body.append(dialog); dialog.showModal(); field.focus(); return dialog;
}
