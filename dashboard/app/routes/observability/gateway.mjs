...
export async function renderRoute(context) {
  ...
  view.node.append(dashboardPanel(document, { title: 'Summary', kind: 'list' }, definitionList([['Requests', number(data.summary.requests)], ['Errors', number(data.summary.errors)], ['Error rate', `${data.summary.errorRate}%`], ['Median latency', `${data.summary.latencyP50 ?? 'Unknown'} ms`], ['P95 latency', `${data.summary.latencyP95 ?? 'Unknown'} ms`], ['Missing latency samples', number(data.summary.missingLatencySamples)]])));
  ...
  const chartPanel = dashboardPanel(document, { title: metrics[selected][0], kind: 'graph' }, nav); const host = make('div'); chartPanel.append(host, link(`/api/operations/export?${new URLSearchParams({ ...Object.fromEntries(context.url.searchParams), view: 'gateway' })}`, 'Export scoped gateway evidence'), make('p', `Fresh as of ${instant(data.freshAsOf)}`, 'freshness')); view.node.append(chartPanel);
