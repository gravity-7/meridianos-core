import { make, link, badge, definitionList, money, number, instant, page, notice, scopeText, table } from '../../shared/view-helpers.mjs';
...
export async function renderRoute(context) {
  ...
  grid.append(
    familyPanel(document, { title: 'Error rate gauge', kind: 'bar-gauge', value: `${data.health.errorRate}%`, percent: data.health.errorRate, caption: `${data.health.requests} requests in scope`, href: data.health.drilldown.href, linkLabel: 'Open gateway evidence' }),
    familyPanel(document, { title: 'Latency heatmap', kind: 'heatmap', rows: latencyRows, emptyMessage: 'No latency samples in this scope.', href: '/app/observability/gateway?metric=latencyP95', linkLabel: 'Open latency evidence' }),
    familyPanel(document, { title: 'Budget signals', kind: 'table', rows: [
    ...
    familyPanel(document, { title: 'Recent activity', kind: 'list', rows: (data.attention ?? []).slice(0, 5).map((item) => ({ label: item.title, value: item.severity })), emptyMessage: 'No recent alerts in this scope.', href: '/app/observability/alerts', linkLabel: 'Open alert list' }),
  );
  ...
  grid.append(panelList(document, 'Open alerts', data.attention, 'No open alerts in this scope.'));
  const driverPanel = dashboardPanel(document, { title: 'Operational snapshot', kind: 'table', className: 'panel-table' }); driverPanel.append(definitionList([['Gateway state', badge(data.health.state, data.health.state)], ['Spend', money(data.cost.spend)], ['Budget period', data.cost.budget?.periodLabel ?? 'Unknown'], ['Usage requests', number(data.usage?.requests ?? 0)]]), link('/app/observability/cost', 'Open cost drivers')); grid.append(driverPanel);
