 ...
 * "Rules" section.
 */
export function buildPrompt(task, { branch, config, model, agent } = {}) {
  ...
  // Model provenance in the PR BODY (companion to the commit-identity stamp in gitIdentityEnv).
  // The commit identity is the agent-proof record; this makes the human-facing PR description carry
  // it too. It is prompt-driven, so it only applies to the PR-producing (implement) stage, and we
  // hand the agent the EXACT line to copy rather than asking it to "mention the model" — a filled-in
  // string is followed far more reliably than an instruction to compose one. `model`/`agent` are
  // optional: when the caller doesn't supply them (spec/design stages, or a bare test call) nothing
  // is added, so existing prompts are byte-identical.
  const provModel = String(model || '').trim();
  if (s.target === 'in-review' && provModel) {
    const line = `🤖 Generated by MeridianOS — model \`${provModel}\`${agent ? ` · agent \`${agent}\`` : ''} · task \`${task.id}\``;
    parts.push(
      '',
      '## PR description',
      'When you open the PR, END the PR body with this line, VERBATIM (it records which model authored the change):',
      '',
      line,
    );
  }
...
export async function launchAgent({ agent, model, task, session, provider, harness, tier, _spawn = spawnAndWait, config }) {
  ...
  try {
    const prompt = buildPrompt(task, { branch: wt.branch, config, model, agent });
