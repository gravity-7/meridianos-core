...
description: Multi-perspective code review with autonomous fix loop - dispatches 6
...
> **Note:** This capability is not available on Codex. This feature requires harness-specific support.
...
- Dispatch each review agent one at a time using subagents
...
Dispatch review agents as subagents when explicitly requested.
Each agent runs in its own context.
If subagents are not available, execute all review perspectives sequentially.
...
**For each agent dispatch**, Use a subagent with:
- a subagent
