...
`;
  ...
  // --- .ai/providers.yaml (003 — Provider & Model Agnosticism) ---
  // Generate from built-in defaults, auto-uncommenting providers whose keyEnv is set
  const defaultsPath = join(HERE, 'providers.defaults.yaml');
  let providersYaml = '';
  if (existsSync(defaultsPath)) {
    try {
      const defaultsRaw = readFileSync(defaultsPath, 'utf8');
      const defaultsParsed = parseYaml(defaultsRaw);
      const providers = defaultsParsed?.providers ?? {};

      // Build providers.yaml, auto-detecting keys from environment
      const detected = [];
      const providerEntries = [];
      for (const [name, def] of Object.entries(providers)) {
        const keySet = def.keyEnv && process.env[def.keyEnv];
        if (keySet) detected.push(`${name} (${def.keyEnv})`);

        providerEntries.push(`  ${name}:`);
        providerEntries.push(`    name: ${name}`);
        if (def.displayName) providerEntries.push(`    displayName: "${def.displayName}"`);
        providerEntries.push(`    wire: ${def.wire}`);
        providerEntries.push(`    baseUrl: "${def.baseUrl}"`);
        providerEntries.push(`    keyEnv: ${def.keyEnv ?? 'null'}`);
        if (def.auth) {
          providerEntries.push(`    auth:`);
          providerEntries.push(`      mode: ${def.auth.mode ?? 'env'}`);
        }
        if (def.features && Object.keys(def.features).length > 0) {
          providerEntries.push('    features:');
          for (const [fk, fv] of Object.entries(def.features)) {
            providerEntries.push(`      ${fk}: ${fv}`);
          }
        }
      ...
      const header = [
        '# MeridianOS — local provider state (auto-generated by init)',
        '# This file is the MIDDLE priority in the three-source merge:',
        '#   policy.yaml > .ai/providers.yaml > providers.defaults.yaml',
        '# Edit policy.yaml to override any field; re-run init to regenerate this file.',
        '# Generated: ' + new Date().toISOString(),
        '',
      ];

      if (detected.length > 0) {
        header.push(`# Auto-detected providers (key present in environment): ${detected.join(', ')}`);
        header.push('');
      }

      providersYaml = header.join('\n') + 'providers:\n' + providerEntries.join('\n');

      writeFileSync(join(aiDir, 'providers.yaml'), providersYaml, 'utf8');
      console.log('  ✓ .ai/providers.yaml');
      if (detected.length > 0) {
        console.log(`    Auto-detected: ${detected.join(', ')}`);
      }
    } catch (err) {
      console.log(`  ⚠ .ai/providers.yaml skipped (${err.message})`);
    }
  }
