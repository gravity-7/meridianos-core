...
| **C6 · L2 packaging** | A | ⬜ | `Dockerfile` (new), `docker-compose.yml` (new), `.dockerignore` (new), `docs/DEPLOY.md` (new) | — | C-tenant shape stable |
| **C7 · mos-dev thin-tenant rebuild** | A | ⬜ | *(mos-dev repo)* `tools/aios/*`, `.ai/policy.yaml`, `package.json`, `.npmrc` | *(separate repo — no core chokepoint)* | — |
