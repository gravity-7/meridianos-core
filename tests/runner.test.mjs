...
test('planRun fires with a claimable decision, and not when held', () => {
  ...
  const ok = planRun({ store, policy: policy(), budget: budget(), now: localAt(12), runs: [], config });
  assert.equal(ok.fire, true);
  assert.equal(ok.decisions.find((d) => d.agent === 'claude').task.id, 'F-impl');
  const held = planRun({ store, policy: policy(), budget: budget({ kill_switch: true }), now: localAt(12), runs: [], config });
...
test('planRun reports nothing_to_claim when no task is eligible', () => {
  const p = planRun({ store: freshStore([]), policy: policy(), budget: budget(), now: localAt(12), runs: [], config });
...
test('executeRun without a launcher is a dry run — nothing claimed or logged', async () => {
  ...
  const r = await executeRun({ store, policy: policy(), budget: budget(), now: localAt(12), runs: [], runsPath, config });
...
test('executeRun with a launcher claims the task and logs the run', async () => {
  ...
  const r = await executeRun({ store, policy: policy(), budget: budget(), now: localAt(12), runs: [], runsPath, launch, agents: ['claude'], config });
...
test('executeRun threads the launcher\'s usage object into the run log (1.6)', async () => {
  ...
  await executeRun({ store, policy: policy(), budget: budget(), now: localAt(12), runs: [], runsPath, launch, agents: ['claude'], config });
  const log = readRuns({ path: runsPath });
...
test('executeRun skips a task routed to a provider with an unset key — never launches, logs + warns', async () => {
  ...
  try {
    const r = await executeRun({ store, policy: p, budget: budget(), now: localAt(12), runs: [], runsPath, launch, agents: ['claude'], config });
    assert.equal(r.fired, false);
