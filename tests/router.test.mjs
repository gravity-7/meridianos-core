...
test('happy path (no model_routing at all) resolves to native anthropic + the agent\'s own harness', () => {
  ...
  const da = decide(freshStore([dsgn({ owner: 'both' })]), { agent: 'antigravity', policy: policy(), budget: budget(), config });
  assert.equal(da.provider, 'anthropic');
  assert.equal(da.harness, 'antigravity');
});

test('kill switch denies all claims', () => {
  const d = decide(freshStore([impl()]), { agent: 'claude', policy: policy(), budget: budget({ kill_switch: true }), config });
  assert.equal(d.mayClaim, false);
  assert.equal(d.reason, 'kill_switch');
});

test('budget halt denies claims', () => {
  const d = decide(freshStore([impl()]), { agent: 'claude', policy: policy(), budget: budget({ mayClaim: { claude: false }, claude: { state: 'halt' } }), config });
  assert.equal(d.mayClaim, false);
...
test('auto-downgrade picks the routine model at warn; default otherwise', () => {
  ...
  assert.equal(decide(store, { agent: 'claude', policy: warnPolicy, budget: budget({ claude: { state: 'warn' } }), config }).model, 'claude-haiku-4-5');
  // flag off → stay on default even at warn
  assert.equal(decide(store, { agent: 'claude', policy: policy(), budget: budget({ claude: { state: 'warn' } }), config }).model, 'claude-opus-4-8');
});
...
test('max_parallel blocks new claims when the system is saturated', () => {
  ...
  const d = decide(store, { agent: 'antigravity', policy: policy(), budget: budget(), config });
  assert.equal(d.mayClaim, false);
...
test('wip_per_agent blocks a second concurrent task for one agent', () => {
  ...
  const store = createProjectStore({ db, config });
  claimTask(db, { taskId: 'F-impl', agent: 'claude', session: 's1' });
  const d = decide(store, { agent: 'claude', policy: policy({ work: { max_parallel: 5, wip_per_agent: 1, priority_floor: 999, lease_ttl_min: 30 } }), budget: budget(), config });
  assert.equal(d.mayClaim, false);
  assert.equal(d.reason, 'wip_per_agent');
});

test('priority_floor rejects tasks above the floor number', () => {
  const store = freshStore([impl({ priority: 70 })]);
  const d = decide(store, { agent: 'claude', policy: policy({ work: { max_parallel: 2, wip_per_agent: 1, priority_floor: 10, lease_ttl_min: 30 } }), budget: budget(), config });
  assert.equal(d.mayClaim, false);
  assert.equal(d.reason, 'below_priority_floor');
});

test('no eligible task is reported', () => {
  const d = decide(freshStore([]), { agent: 'claude', policy: policy(), budget: budget(), config });
  assert.equal(d.mayClaim, false);
...
test('capability_matrix allows shared categories for both agents', () => {
  const store = freshStore([impl({ id: 'F-test', risk_tags: ['testing'], owner: 'both' })]);
...
test('work_stealing=false blocks antigravity from claiming claude-owned tasks', () => {
  const store = freshStore([impl({ owner: 'claude', risk_tags: ['backend'] })]);
...
test('tasks with no risk_tags pass the capability filter (open to any agent)', () => {
  const store = freshStore([impl({ owner: 'both', risk_tags: [] })]);
  const p = policy({ capability_matrix: { 'money-math': ['claude'] }, work_stealing: true });
  const da = decide(store, { agent: 'antigravity', policy: p, budget: budget(), config });
  assert.equal(da.mayClaim, true);
});
...
test('antigravity can claim ready-for-impl tasks (expanded CLAIMABLE)', () => {
  const store = freshStore([impl({ owner: 'both', risk_tags: ['testing'] })]);
...
test('claude can claim spec tasks for spec-writing', () => {
  const store = freshStore([{ id: 'F-spec', title: 'needs spec', owner: 'claude', status: 'spec', priority: 10 }]);
  const dc = decide(store, { agent: 'claude', policy: policy(), budget: budget(), config });
  assert.equal(dc.mayClaim, true);
  assert.equal(dc.task.status, 'spec');
});

test('both agents can claim designing tasks', () => {
  const store = freshStore([dsgn({ owner: 'both' })]);
  const dc = decide(store, { agent: 'claude', policy: policy(), budget: budget(), config });
  const da = decide(store, { agent: 'antigravity', policy: policy(), budget: budget(), config });
  assert.equal(dc.mayClaim, true);
  assert.equal(da.mayClaim, true);
});
...
test('object-form model_routing threads provider + harness through decide()', () => {
  ...
  try {
    const store = freshStore([impl({ complexity: 1, risk_tags: [] })]);
    const p = policy({
    ...
    const d = decide(store, { agent: 'claude', policy: p, budget: budget(), config });
    assert.equal(d.mayClaim, true);
...
test('budget-warn downgrade still applies with object-form provider routing', () => {
  ...
  try {
    const store = freshStore([impl({ complexity: 5, risk_tags: [] })]);
    const p = policy({
    ...
    const d = decide(store, { agent: 'claude', policy: p, budget: budget({ claude: { state: 'warn' } }), config });
    assert.equal(d.mayClaim, true);
