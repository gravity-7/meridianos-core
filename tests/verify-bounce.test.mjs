...
test('a failing check BOUNCES the task out of in-review (no infinite re-check loop)', async () => {
  ...
  clearVerifyState('S1');
  const r = await verifyCycle(db, { policy, dryRun: false, checkRunners: failRunners });
  ...
  const r2 = await verifyCycle(db, { policy, dryRun: false, checkRunners: failRunners });
...
test('after MAX attempts a persistently failing task is BLOCKED + escalated, not churned', async () => {
  ...
  for (let i = 0; i < 2; i++) {
    await verifyCycle(db, { policy, dryRun: false, checkRunners: failRunners });
...
test('all checks pass + peer LGTM → dry-run reports a merge', async () => {
  ...
  const r = await verifyCycle(db, { policy: { auto_merge: 'verifier_gated' }, dryRun: true, checkRunners: passRunners });
