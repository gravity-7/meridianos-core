...
- If `ask` is `"smart"` or `"never"`: suppress all prompts, complete the verification autonomously, and return immediately so the pipeline can advance.
...
**If specs found:** Present list and ask user to select one (skip in autonomous mode).
